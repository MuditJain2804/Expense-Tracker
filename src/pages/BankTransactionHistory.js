...
const BankTransactionHistory = ({setBankTransactionType,selectedBankCategory,bankTransaction,bankTransactionName,bankTransactionDescription,bankTransactionAmount,bankTransactionDate}) => {
  const navigate = useNavigate();
  const type = ['Income','Expense'];
  const ArrayLength = (selectedBankCategory.length)
  const categoryIcons = {
  ...
  const Category = selectedBankCategory.slice(0, ArrayLength);
  const transaction = bankTransaction.slice(0, ArrayLength);
  const Name = bankTransactionName.slice(0, ArrayLength);
  const Description = bankTransactionDescription.slice(0, ArrayLength);
  const Amount = bankTransactionAmount.slice(0, ArrayLength);
  const Date = bankTransactionDate.slice(0, ArrayLength).map(date => dayjs(date).format('DD/MM/YYYY hh:mm A')); 
  function clickHandler(transaction){
    navigate('/AddBankTransaction')
    setBankTransactionType(transaction)
  }
  ...
  return (
    <div style={{position:"relative",height:"97vh"}}>
        <div style={{display:"flex", gap:"15px"}}>
            ...
            <div>Bank Transaction History</div>
        </div>
        {ArrayLength>0 ? 
        <div style={{display:"flex",flexDirection:"column",gap:"20px",alignItems:"baseline"}}>
            ...
            {
              bankTransaction[index] === "Income" ? <div style={{color:"green"}}>₹{Amount[index]}</div> : <div style={{color:"red"}}>₹{Amount[index]}</div>
            }
            </div>  
            )}
        </div> : 
