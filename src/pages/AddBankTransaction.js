...
const AddBankTransaction = ({bankTransactionType,addedCategories,selectedBankCategory,setSelectedBankCategory,bankTransaction,setBankTransaction,bankTransactionName,setBankTransactionName,bankTransactionDescription,setBankTransactionDescription,bankTransactionAmount,setBankTransactionAmount,bankTransactionDate,setBankTransactionDate,bankIncome,setBankIncome,bankExpense,setBankExpense,spentCategory,setSpentCategory,spentAmount,setSpentAmount,incomeCategory,setIncomeCategory,incomeAmount,setIncomeAmount}) => {
  const navigate = useNavigate();
  const [selectedValue,setSelectedValue] = useState(bankTransactionType)
  ...
  function clickHandler(category){
    ...
    if(selectedValue === 'Income') {
      setBankIncome(bankIncome + amount)
    }
    if(selectedValue === 'Expense') {
      setBankExpense(bankExpense + amount)
    } 
  }
  ...
  function submitHandler(event){
    ...
    setBankTransaction([...bankTransaction,selectedValue])
    setBankTransactionName([...bankTransactionName,name])
    setBankTransactionDescription([...bankTransactionDescription,description])
    setBankTransactionAmount([...bankTransactionAmount,amount])
    setBankTransactionDate([...bankTransactionDate,date])
    if (selectedCategory) {
      setSelectedBankCategory([...selectedBankCategory,selectedCategory])
    }
    if(budgetCategory){
